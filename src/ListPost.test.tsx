...
const fetchError: any = async () => {
  return Promise.resolve({
    json: () => Promise.reject("Someting went wrong"),
  });
};

configure({ testIdAttribute: "data-test-id" }); 

test("renders List Post Components", () => {
  act(() => render(<ListPosts />));
});

test("Render data from API", async () => {
  jest.spyOn(window, "fetch").mockImplementation(() => fetchData());
...
test("I can handle Error in catch block", () => {
  jest.spyOn(window, "fetch").mockImplementation(() => fetchError());

  render(<ListPosts />);
});

test("I can Add the record", async () => {
  jest.spyOn(window, "fetch").mockImplementation(() => fetchData());
...
test("I can Delete the record", async () => {
  jest.spyOn(window, "fetch").mockImplementation(() => fetchData());
...
test("I can Edit the record", async () => {
  jest.spyOn(window, "fetch").mockImplementation(() => fetchData());

  render(<ListPosts />);

  await waitFor(() => {
    const editPost = screen.getByTestId("edit-post-1");
    fireEvent.click(editPost);
  });
});

test("I can submit the new Post", async () => {
  ...
  act(() =>
    fireEvent.click(submitButton, {
      ...
      target: { title: { value: "Test" }, description: { value: "Test" } },
    })
  );
  ...
  // onClose functions covered by Escape Button Pressed
  const closeModal = getByTestId("postModal");
  act(() =>
...
test("I can close the modal", async () => {
  ...
  waitFor(() => {
    const closeModal = getByText("Save Post");
    act(() =>
